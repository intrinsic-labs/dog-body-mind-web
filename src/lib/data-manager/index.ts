...
export class DataManager implements IDataManager {
  ...
  async initialize(): Promise<void> {
    dmLog(`🔍 DataManager: Initializing for language: ${this.language}`);
    try {
      // Eagerly load organization (always needed for schema.org)
      dmLog("📡 DataManager: Fetching organization...");
      ...
      if (!organization) {
        dmLog(
          "🐛 DataManager: Trying debug query without language filtering...",
        );
        const debugOrg = await client.fetch(organizationDebugQuery);
        dmLog("🐛 DataManager: Debug organization result:", debugOrg);
      }
    ...
    try {
      ...
      if (post.categories && post.categories.length > 0) {
        ...
        const resolvedCategories =
          await this.getMultipleReferences(categoryRequests);

        // Maintain the original order from post.categories
        for (const postCat of post.categories) {
          const resolved = resolvedCategories.find(
            (r) => r.id === postCat._ref,
          );
  ...
  async getAllPosts(): Promise<AllPostsQueryResult> {
    ...
    try {
      const posts = await client.fetch<AllPostsQueryResult>(allPostsQuery, {
        language: this.language,
      });
      return posts;
    } catch (error) {
